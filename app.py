...
MARKET_PENALTIES = {
    ...
    "Sin gol 0-10 + gol antes del 70": 0.0,
    "Más de 0.5 goles en 1ª parte": 1.0,
    "Local marca antes del 70": 1.0,
    "Visitante marca antes del 70": 1.0,
...
def finished(item: dict[str, Any]) -> bool:
    return ((item.get("fixture", {}).get("status", {}) or {}).get("short")) in {
        "FT", "AET", "PEN"
    }
...
def minute_metrics(
    ...
    n = no_goal_10 = goal_before_70 = goal_1h = 0
    team_goal_70 = opp_goal_70 = 0
    team_first = opp_first = 0
    ...
    for item in valid:
        ...
        goal_1h += int(any(minute <= 45 for minute, _ in goals))
        team_goal_70 += int(
...
def analyze_fixture(
    ...
    home_venue = venue_fixtures(
        home_all,
        int(home_id),
        home=True,
        limit=RECENT_VENUE,
    )
    away_venue = venue_fixtures(
        away_all,
        int(away_id),
        home=False,
        limit=RECENT_VENUE,
    ...
    home_base = home_venue or home_all[:RECENT_GENERAL]
    away_base = away_venue or away_all[:RECENT_GENERAL]
    ...
    if "Goles" in enabled_groups:
        rows.extend([
            score_market("Goles", "Total de goles - Más de 1.5",
                         avg(hm.get("over15", 0), am.get("over15", 0)), sample),
            score_market("Goles", "Total de goles - Más de 2.5",
                         avg(hm.get("over25", 0), am.get("over25", 0)), sample),
            score_market("Goles", "Total de goles - Menos de 3.5",
                         avg(hm.get("under35", 0), am.get("under35", 0)), sample),
            score_market("Goles", "Total de goles - Menos de 4.5",
                         avg(hm.get("under45", 0), am.get("under45", 0)), sample),
            score_market("Goles", "Goles del local - Más de 0.5",
                         avg(hm.get("score", 0), am.get("concede", 0)), sample),
            score_market("Goles", "Goles del visitante - Más de 0.5",
                         avg(am.get("score", 0), hm.get("concede", 0)), sample),
            score_market("Goles", "Goles del local - Más de 1.5",
                         avg(hm.get("team_over15", 0), am.get("concede", 0)), sample),
            score_market("Goles", "Goles del visitante - Más de 1.5",
                         avg(am.get("team_over15", 0), hm.get("concede", 0)), sample),
            score_market("Goles", "Goles del local - Menos de 2.5",
                         avg(hm.get("team_under25", 0), 100 - am.get("concede", 0) / 2), sample),
            score_market("Goles", "Goles del visitante - Menos de 2.5",
                         avg(am.get("team_under25", 0), 100 - hm.get("concede", 0) / 2), sample),
        ])

    if "BTTS" in enabled_groups:
        rows.extend([
            score_market("BTTS", "Ambos equipos marcan - Sí",
                         avg(hm.get("btts", 0), am.get("btts", 0)), sample),
            score_market("BTTS", "Ambos equipos marcan - No",
            ...
            if "Minutos" in enabled_groups:
                rows.extend([
                    ...
                    score_market(
                        ...
                        avg(
                            hmin["team_goal_before_70"],
                            amin["opp_goal_before_70"],
                        ),
                        event_sample,
                    ),
                    score_market(
                        ...
                        avg(
                            amin["team_goal_before_70"],
                            hmin["opp_goal_before_70"],
                        ),
                        event_sample,
                    ),
                ])
            ...
            if "Primer gol" in enabled_groups:
                rows.extend([
                    score_market(
                        ...
                        "Local marca primero",
                        avg(
                            hmin["team_scores_first"],
                            amin["opponent_scores_first"],
                        ),
                        event_sample,
                    ),
                    score_market(
                        ...
                        avg(
                            amin["team_scores_first"],
                            hmin["opponent_scores_first"],
                        ),
                        event_sample,
                    ),
                ])
    ...
    summary = {
        ...
        "Alternativa 2": second["Mercado"] if second is not None else "—",
        "Score 2": float(second["JOYA Score"]) if second is not None else 0.0,
        "Alternativa 3": third["Mercado"] if third is not None else "—",
...
st.caption("Versión 0.6 · Todos los mercados + Minute Engine + Próximo gol")
...
enabled_group_list = st.sidebar.multiselect(
    "Categorías",
    options=[
        "Goles",
        "BTTS",
...
enabled_groups = set(enabled_group_list)
...
tab_scanner, tab_single, tab_cart, tab_live = st.tabs(
    [
        "🧠 Scanner",
        "📊 Analizar partido",
        "📋 Crear cartilla",
...
with tab_scanner:
    ...
    if "ranking_v06" not in st.session_state:
        st.session_state.ranking_v06 = pd.DataFrame()

    if st.button(
        "🧠 ESCANEAR TODAS LAS LIGAS",
        type="primary",
        use_container_width=True,
    ):
        if not enabled_groups:
            ...
            with st.spinner("Analizando ligas y mercados…"):
                st.session_state.ranking_v06 = scan_all_by_league(
                    prepared,
                    int(max_per_league),
                    enabled_groups,
                    update,
    ...
    ranking = st.session_state.ranking_v06
    ...
    if not ranking.empty:
        ...
        for (country, league), group in ranking.groupby(
            ...
            with st.expander(
                f"{country} · {league} · {len(group)} picks · "
...
with tab_single:
    ...
    if not labels:
        ...
        label = st.selectbox(
            ...
            list(labels.keys()),
            key="single_match_select",
        )

        selected_item = prepared[labels[label]]

        st.caption(
            "Puedes incluir Minutos y Primer gol desde el menú lateral. "
            "Esos módulos consumen más solicitudes porque revisan eventos históricos."
        )
        ...
        if st.button(
            ...
            with st.spinner(
                "Analizando forma, localía, visitante y mercados seleccionados…"
            ):
...
with tab_cart:
    ranking = st.session_state.get("ranking_v06", pd.DataFrame())
    ...
    if st.button(
        ...
        if cart.empty:
            ...
            if combined < target_odds:
                st.warning(
                    f"Cuota estimada {combined:.2f}, bajo el objetivo. "
                    "JOYA no forzó picks."
                )
            else:
                st.success(
...
with tab_live:
    ...
    if not live_matches:
        st.info("No hay partidos en vivo con cobertura activa en esta fecha.")
    else:
        live_labels = {}
        ...
        for index, item in enumerate(live_matches):
            ...
            live_labels[
                f"{league.get('name', '')} · {home} vs {away} · {elapsed or '—'}'"
            ] = index

        selected_live_label = st.selectbox(
            "Selecciona un partido en vivo",
            list(live_labels.keys()),
        )

        selected_live = live_matches[live_labels[selected_live_label]]
        ...
        if st.button(
            ...
            st.caption(
                "Lectura de presión relativa basada en tiros al arco, tiros totales, "
                "córners, posesión y expulsiones cuando existe cobertura."
            )

st.caption(
    "JOYA 20 v0.6 mantiene Scanner, análisis por ligas, mercados generales, "
    "Minute Engine, Primer gol, Cartillas y Próximo gol en vivo dentro de una sola app."
)
