...
st.caption(
    "v1.1 Projection Center · Rangos de goles · Córners · Tarjetas · Remates · BTTS 1T"
)
...
settings.setdefault("advanced_projections", True)
settings.setdefault("projection_sample", 3)
    ...
    for (country, league), league_group in ranking.groupby(
        ...
        with st.expander(f"{country} · {league}", expanded=False):
            for _, match in league_group.iterrows():
                ...
                if settings.get("advanced_projections", True):
                    with st.spinner(
                        f"Calculando proyecciones avanzadas de {match['Local']} vs {match['Visitante']}…"
                    ):
                        advanced = build_advanced_projection(
                            fixture,
                    ...
                    st.markdown("### 🟨 Árbitro y tarjetas")
                    st.info(
                    ...
                    if projection_rows.empty:
                        ...
                        unavailable = projection_rows[
                            projection_rows["Estado"] == "NO BET"
                        ]

                        if not unavailable.empty:
                            st.caption(
                                "Los mercados sin cobertura suficiente se mantienen "
                                "como NO BET; JOYA no inventa porcentajes."
                            )
                else:
                    st.info(
                        "Activa las proyecciones avanzadas en la barra lateral "
                        "para calcular córners, tarjetas y remates."
                    )
